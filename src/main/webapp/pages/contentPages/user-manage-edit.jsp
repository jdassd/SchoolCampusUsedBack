...
<%--                        <option value="" selected="selected">男</option>--%>
                ...
                <div class="col-md-4 data">
                    <input type="text" class="form-control" placeholder="wait..." value="" disabled id="emailDiv" name="email">
                </div>
...
<script>
    ...
    function initNums() {
        ...
        $.get("/userManageServlet",{"id":id[id.length-1]},function (data) {
            $(data).each(function () {
                $("#userIdDiv").val(this.userId);
                $("#nameDiv").val(this.name);
                $("#usernameDiv").val(this.username);
                $("#passwrodDiv").val(this.password);
                $("#emailDiv").val(this.email);
                if(this.sex == 1){
                    $("#sexSelect>option:first-child").attr("selected","selected");
                    $("#select2-sexSelect-container").html("男");
                }else{
                    $("#sexSelect>option:last-child").attr("selected","selected");
                    $("#select2-sexSelect-container").html("女");
                }
                if(this.status == 1){
                    $("#statusSelect>option:first-child").attr("selected","selected");
                    $("#select2-statusSelect-container").html("启用");
                }else{
                    $("#statusSelect>option:last-child").attr("selected","selected");
                    $("#select2-statusSelect-container").html("停封");
                }
                if(this.admin == 1){
                    $("#adminSelect>option:first-child").attr("selected","selected");
                    $("#select2-adminSelect-container").html("是");
                }else{
                    $("#adminSelect>option:last-child").attr("selected","selected");
                    $("#select2-adminSelect-container").html("否");
                }
            });
        },"json");
    }
    ...
    function update() {
        if(check()){
            ...
            $.post("../userManageServlet",json,function () {
                //重新加载数据
                initNums();
            },"json");
